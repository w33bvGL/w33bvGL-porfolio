...
const handleSubmit = async () => {
  try {
    if (timerStore.isCooldown) {
      alert(t('validation.cooldownMessage', { time: remainingTime.value }));
      return;
    }
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
      <div class="flex flex-col">
        ...
        <ul class="mt-4 space-y-4">
          ...
          <li><strong class="font-medium">{{ t('contact.phone') }}:</strong> <UButton variant="link" :padded="false" to="tel:+37494161331">+374(94) 161-331</UButton></li>
          <li><strong class="font-medium">{{ t('contact.email') }}:</strong> <UButton variant="link" :padded="false" to="mailto:w33bv.gl@gmail.com">w33bv.gl@gmail.com</UButton></li>
        </ul>
      </div>
      <div class="flex flex-col">
        <h2 class="text-3xl md:text-4xl font-bold mb-4 text-center md:text-left">
        ...
        <UForm @submit.prevent="handleSubmit" :schema="schema" :state="state">
          <UFormGroup :label="t('contact.form.nameLabel')" name="name" size="xl">
            <UInput v-model="state.name" size="xl" :placeholder="t('contact.form.namePlaceholder')" required />
          </UFormGroup>
          <UFormGroup :label="t('contact.form.emailLabel')" name="email" class="mt-4" size="xl">
            <UInput v-model="state.email" size="xl" :placeholder="t('contact.form.emailPlaceholder')" required />
          </UFormGroup>
          <UFormGroup :label="t('contact.form.messageLabel')" name="message" class="mt-4" size="xl">
            <UTextarea v-model="state.message" size="xl" :placeholder="t('contact.form.messagePlaceholder')" required />
          </UFormGroup>
          <UButton
              block
          ...
          {{ timerStore.isCooldown
            ? t('contact.form.waitMessage', { time: formatTime(timerStore.remainingTime) })
