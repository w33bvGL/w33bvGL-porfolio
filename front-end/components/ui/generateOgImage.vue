...
const { data: profile } = await useAsyncData<Profile>("github-profile-og", () =>
...
const generateOgImage = async () => {
  ...
  avatar.onload = () => {
    ...
    ctx.fillRect(200, 200, (profile.value.public_repos / totalRepos) * reposBarWidth, 20);
    ...
    if (languages.value) {
      ...
      for (const language of languageKeys) {
        ...
        ctx.fillStyle = "#333";
        ctx.font = "24px Arial";
        ctx.fillText(`${language}: ${count} репозиториев`, 200, yPosition);
        ...
        ctx.fillStyle = "#42b983";
        ctx.fillRect(200, yPosition + 30, (count / totalRepos) * languageBarWidth, 15);
